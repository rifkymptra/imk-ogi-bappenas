<template>
  <div id="app">
    ...
    <TitleSection
      ...
      heading="Dokumen dan Publikasi Data"
      subheading1=""
      ...
      <div class="w-full max-w-[1166px] flex">
        ...
        <div class="w-1/4">
          <div class="p-4 bg-white shadow rounded-lg">
            <h3 class="font-inter font-semibold text-[12px] mb-2">
              Urutkan Berdasarkan
            </h3>
            ...
            <h3 class="font-inter font-semibold text-[12px] mb-3 mt-4">
              Tahun
            </h3>
            <Slider
              v-model="yearRange"
        ...
        <div class="w-3/4 pl-4">
          ...
          >
            <p class="font-inter font-bold text-[18px] text-center">
          ...
          <div v-else class="space-y-4">
            ...
            >
              <h3
                class="font-inter font-semibold text-[12px] text-primary-1 mb-2"
              >
                {{ report.kategori }}
              </h3>
              ...
              <div class="flex justify-between items-center mt-2">
                <a :href="report.link" target="_blank">
                  <button
                    class="w-[79px] h-[32px] font-jakarta font-bold text-[13px] bg-primary-1 text-white rounded-lg hover:bg-primary-3 focus:outline-none"
                  >
                    Baca
                  </button>
                </a>
                <div class="text-gray-500 text-sm flex space-x-4">
                  <span
                    class="flex items-center font-jakarta font-semibold text-[10px] text-neutral-3"
                  >
                    <i data-feather="calendar" class="icon-calendar mr-1"></i>
                    {{ report.tanggal }}
                  </span>
                  <span
                    class="flex items-center font-jakarta font-semibold text-[10px] text-neutral-3"
                  >
                    <i data-feather="eye" class="icon-eye mr-1"></i>
          ...
          >
            ...
            <button
              ...
              :class="{
                'bg-white border-primary-1 text-primary-1':
                  currentPage === page,
                'bg-white border-neutral-3 text-neutral-3':
                  currentPage !== page,
              }"
            ...
            <button
              ...
              :class="{
                'bg-neutral-4 text-white cursor-not-allowed':
...
import Dokumen from "../../assets/data/dokumen.json";
...
export default {
  ...
  data() {
    return {
      ...
      reports: Dokumen, // Load the reports data from JSON
      filteredReports: Dokumen, // To store filtered reports
  ...
  methods: {
    ...
    applyFilter() {
      console.log(
        `Applying filter: Sorting by ${this.sortCriteria}, Year range: ${this.yearRange}, Category: ${this.selectedCategory}, Search Query: ${this.searchQuery}`
      );

      let filtered = this.reports.filter((report) => {
        const isInYearRange =
          report.tahun >= this.yearRange[0] &&
          report.tahun <= this.yearRange[1];
        const isInCategory =
          this.selectedCategory === "Semua" ||
          report.kategori === this.selectedCategory;
        const matchesSearchQuery =
          report.judul.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
          report.kategori
            .toLowerCase()
